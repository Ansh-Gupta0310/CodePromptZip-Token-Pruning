"""
evaluate.py - Full Evaluation Pipeline for CodePromptZip

End-to-end evaluation:
1. Load test data
2. Retrieve code examples via BM25
3. Compress retrieved examples using trained compressor
4. Construct RAG prompts
5. Pass to Base LM (CodeLlama-34B-Instruct) for generation
6. Evaluate with task-specific metrics (Exact Match / CodeBLEU)

This implements the evaluation for RQ1 (comparison with baselines)
and reproduces the results from Table 2 of the paper.
"""
...
class BaseLMInference:
    ...
    Uses CodeLlama-34B-Instruct (quantized GGUF) for local inference.
    """
    ...
    def __init__(self, config: Dict):
        ...
        if not os.path.exists(model_path):
            ...
            print("[BaseLM] Please download CodeLlama-34B-Instruct GGUF:")
            print("  Download from: https://huggingface.co/TheBloke/CodeLlama-34B-Instruct-GGUF")
            print("  Recommended file: codellama-34b-instruct.Q4_K_M.gguf")  
        ...
        try:
            ...
            self.model = Llama(
                ...
                n_gpu_layers=config.get("n_gpu_layers", 128),
                verbose=False,
            )
            ...
            print(f"[BaseLM] WARNING: Failed to create context ({e}).")
            print("[BaseLM] Falling back to dummy mode.")
            self.dummy_mode = True
...
class Evaluator:
    ...
    def __init__(
        ...
        # Defer loading CodeLlama — it will be loaded after compression
        # finishes and the compressor is unloaded from GPU.
        self._base_lm_override = base_lm
        self._base_lm = None
        self.retriever = BM25Retriever()

    @property
    def base_lm(self):
        """Lazy-load CodeLlama only when first needed (after compressor is freed)."""
        if self._base_lm is None:
            self._base_lm = self._base_lm_override or BaseLMInference(self.config["base_lm"])
    ...
    def run_evaluation(
        ...
        # Step 4: Free compressor from GPU before loading CodeLlama
        if self.compressor is not None:
            print("\n[3.5/5] Unloading compressor to free GPU for CodeLlama...")
        ...
        # Step 5: Construct RAG prompts and generate (CodeLlama loads here on first access)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate CodePromptZip")
    ...
    # Load compressor on GPU — it will be unloaded before CodeLlama is loaded.
    compressor = None
    if args.checkpoint:
