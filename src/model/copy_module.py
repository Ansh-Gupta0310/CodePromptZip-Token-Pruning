...
class CopyModule(nn.Module):
    ...
    def forward(
        ...
        # ── Dtype guard: ensure all tensors match the linear layer dtype ──
        # T5-large on GPU with autocast returns Half activations, but
        # gen_linear weights are Float. Cast everything to match the module.
        target_dtype = self.gen_linear.weight.dtype
        decoder_hidden_states = decoder_hidden_states.to(target_dtype)
        cross_attention_weights = cross_attention_weights.to(target_dtype)
        encoder_hidden_states = encoder_hidden_states.to(target_dtype)
        vocab_logits = vocab_logits.to(target_dtype)
