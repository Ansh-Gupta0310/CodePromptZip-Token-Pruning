...
# Allow running directly: python src/model/copy_codet5.py
...
class CopyCodeT5(nn.Module):
    ...
    def __init__(
        ...
        # Load base T5 model.
        # - For HuggingFace Hub IDs (e.g. "Salesforce/codet5-large"):
        #   use_safetensors=False because the Hub repo only ships pytorch_model.bin,
        #   and the auto-conversion background thread would error trying to create a PR.
        # - For local checkpoint directories (e.g. "./checkpoints/best_model"):
        #   use_safetensors=None (auto-detect) because newer transformers saves
        #   local checkpoints as model.safetensors by default.
        _is_local = os.path.isdir(model_name)
        self.t5 = T5ForConditionalGeneration.from_pretrained(
            model_name,
            use_safetensors=None if _is_local else False,
        )
...
def create_model(
    ...
    if gradient_checkpointing:
        # use_reentrant=False is required for compatibility with torch.amp autocast + GradScaler.
        # With use_reentrant=True (legacy default), the recomputed forward pass during backward
        # inherits the autocast FP16 context, producing FP16 gradients → GradScaler crashes.
...
if __name__ == "__main__":
    ...
    test_model_name = "Salesforce/codet5-large"
