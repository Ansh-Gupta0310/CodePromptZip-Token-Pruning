...
# Allow running directly: python src/compress.py
...
class CodeCompressor:
    ...
    def __init__(
        ...
        try:
            ...
            # Handle HuggingFace Linux bug where additional_special_tokens is saved incorrectly
            from src.tokenizer_utils import get_extended_tokenizer
            self.tokenizer = get_extended_tokenizer("Salesforce/codet5-large")
    ...
    def compress_demonstration(
        ...
        if task == "bugs2fix":
            # Compress the full demonstration as a combined string — this matches
            # the training format (dataset_construction.py line 84) where
            # buggy+fixed are concatenated and compressed together so the
            # priority-based removal operates on the global token pool.
            buggy_text = demo.get("buggy", "")
            fixed_text = demo.get("fixed", "")
            ...
            if "[FIXED_CODE]" in compressed:
                # Markers survived compression — split on them directly
            ...
            elif "[BUGGY_CODE]" in compressed:
                # Only BUGGY marker survived — all remaining text is from buggy
                text = compressed.replace("[BUGGY_CODE]", "").strip()
                buggy_ratio = len(buggy_text.split()) / max(len(buggy_text.split()) + len(fixed_text.split()), 1)
                tokens = text.split()
                split_idx = int(len(tokens) * buggy_ratio)
                compressed_demo["buggy"] = " ".join(tokens[:split_idx])
                compressed_demo["fixed"] = " ".join(tokens[split_idx:])
            else:
                # Neither marker survived — split proportionally by original token counts
                buggy_tokens = len(buggy_text.split())
                fixed_tokens = len(fixed_text.split())
                total = buggy_tokens + fixed_tokens
                if total > 0:
                    buggy_ratio = buggy_tokens / total
