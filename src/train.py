...
Optimized for RTX 4090 (24GB VRAM) with:
- Mixed precision (FP16)
- Large batch size (no gradient accumulation needed)
- Full activation storage (gradient_checkpointing disabled)

Hyperparameters from paper (Section 6), scaled for RTX 4090:
    Optimizer: AdamW
    LR: 5e-5
    Warmup: 500 steps
    Epochs: 10
...
# Prevent memory fragmentation on 24GB GPU (recommended by PyTorch for large models)
...
# Allow running directly: python src/train.py --config configs/config.yaml
...
class Trainer:
    ...
    def __init__(
        ...
        # Ensure all parameters are FP32 for mixed-precision training.
        # GradScaler expects FP32 parameters; autocast handles FP16 in the forward pass.
        # Some parameters may load as FP16 from from_pretrained() — .float() fixes this.
        self.model.float()
        ...
        self.train_loader = DataLoader(
            ...
            shuffle=True,
            num_workers=4,  # Linux: use multiple workers for faster data loading
            pin_memory=True,
            drop_last=True,
        )
