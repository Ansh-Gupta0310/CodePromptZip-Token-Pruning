...
# Allow running directly: python src/dataset_construction.py
...
def main():
    ...
    parser.add_argument("--config", type=str, default=None,
                        help="Path to config.yaml (overrides all other defaults)")
    ...
    # ── Load from config.yaml if provided (same as train.py / evaluate.py) ──
    if args.config:
        with open(args.config, "r") as f:
            cfg = yaml.safe_load(f)
        # Config values are used as defaults; CLI args override them if given
        raw_data_dir = args.raw_data_dir if args.raw_data_dir != "./data/raw" \
            else cfg["data"]["raw_dir"]
        output_dir = args.output_dir if args.output_dir != "./data/compression_dataset" \
            else cfg["data"]["compression_dataset_dir"]
        task = args.task or cfg["data"]["task"]
        seed = args.seed if args.seed is not None else cfg["training"].get("seed", 42)
        compression_ratios = cfg["data"].get(
            "compression_ratios", [round(i * 0.1, 1) for i in range(1, 10)]
        )
    else:
        raw_data_dir = args.raw_data_dir
        output_dir   = args.output_dir
        task         = args.task or "bugs2fix"
        seed         = args.seed if args.seed is not None else 42
        compression_ratios = [round(i * 0.1, 1) for i in range(1, 10)]

    print("=" * 60)
    print(f"Building Compression Dataset for: {task}")
    if args.config:
